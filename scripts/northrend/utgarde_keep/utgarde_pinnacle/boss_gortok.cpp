...
/*######
## boss_gortok // PARTIE QUI GERE LE BOSS GORTOK .
######*/
...
struct MANGOS_DLL_DECL boss_gortokAI : public ScriptedAI
{
    ...
    void Reset()
    {
        uiArcingSmashTimer = 15000; // TEMPS DE SMASH
        uiImpaleTimer = 12000;      // TEMPS DU SORT IMPALE
        uiWhiteringRoarTimer = 10000;    // TEMPS DE WHITERING ROAR
        ...
        if (m_pInstance)
        {
            ...
            Creature* pTemp;
            if ((pTemp = m_pInstance->instance->GetCreature(m_pInstance->XXXXXXXX(NPC_XXXXXX))) && pTemp->isDead())
                pTemp->Respawn();
            if ((pTemp = m_pInstance->instance->GetCreature(m_pInstance->XXXXXXX(NPC_XXXXXX))) && pTemp->isDead())
                pTemp->Respawn();
            if ((pTemp = m_pInstance->instance->GetCreature(m_pInstance->XXXXXX(NPC_XXXXXX))) && pTemp->isDead())
                pTemp->Respawn();
            if ((pTemp = m_pInstance->instance->GetCreature(m_pInstance->XXXXXX(NPC_FURLBORG))) && pTemp->isDead())
                pTemp->Respawn();
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        if (!m_creature->SelectHostileTarget() || !m_creature->getVictim()) // CETTE PARTIE PERMET AU BOSS D'ATTAQUER LES PERSONNAGES HOSTILE PRES DE LUI .
            return;
...
struct MANGOS_DLL_DECL npc_worgenAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (uiEnrage1Timer <= uiDiff)
        {
            ...
            uiEnrage1Timer = 15000; // TEMPS DU BUFF ENRAGER
        }
        else
        ...
        if (uiEnrage2Timer <= uiDiff)
        {
            ...
            uiEnrage2Timer = 10000;   // TEMPS DU BUFF ENRAGER N°2
        }
        else
